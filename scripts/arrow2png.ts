...
// Minimal Node ambient declarations so this file compiles without
// @types/node. Only the surfaces we actually call.
...
interface NodeBuffer { length: number; }
declare const Buffer: { from(input: string, encoding: string): NodeBuffer };
...
const fs = require('fs') as {
  ...
  writeFileSync(path: string, data: NodeBuffer): void;
};
...
interface ArrowObject { id: string; type: 'arrow'; from: Vec; to: Vec; color: string; thickness: number; }
interface TextObject { id: string; type: 'text'; pos: Vec; text: string; fontSize: number; color: string; }
interface HighlighterObject { id: string; type: 'highlighter'; points: Vec[]; color: string; thickness: number; }
type SceneObject = ArrowObject | TextObject | HighlighterObject;
interface SceneData {
  id: string;
  name: string;
  centerText: string;
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === '--padding') padding = parseInt(args[++i], 10);
...
function usage(code: number): never {
  process.stderr.write(
    'usage: arrow2png <input.json> <output.png> [--width N] [--height N] [--padding N] [--bg #fafafa]\n',
  );
  process.exit(code);
...
type AnyCtx = { [k: string]: any };
type CanvasMod = {
  createCanvas(w: number, h: number): { getContext(type: '2d'): AnyCtx; toBuffer(mime: 'image/png'): NodeBuffer; width: number; height: number };
};

function loadCanvas(): CanvasMod {
  try {
    ...
    return require('canvas') as CanvasMod;
  } catch (e) {
...
function contentBounds(scene: SceneData, padding: number): { x: number; y: number; w: number; h: number; cx: number; cy: number } {
  ...
  if (!hasContent) {
    minX = -100; minY = -60; maxX = 100; maxY = 60;
  }
...
function drawHighlighter(ctx: AnyCtx, hl: HighlighterObject): void {
  ...
  if (hl.points.length === 1) {
    ...
    for (let i = 1; i < hl.points.length; i++) ctx.lineTo(hl.points[i].x, hl.points[i].y);
  }
  ctx.stroke();
  ctx.restore();
}

function drawArrow(ctx: AnyCtx, arrow: ArrowObject): void {
...
function drawText(ctx: AnyCtx, t: TextObject): void {
...
function drawCenter(ctx: AnyCtx, scene: SceneData, cx: number, cy: number): void {
...
function wrapText(ctx: AnyCtx, text: string, cx: number, cy: number, maxWidth: number, lineHeight: number): void {
...
function wrapCenterLines(text: string, maxWidth: number, charWidth: number): string[] {
  const words = text.split(/\s+/);
  const lines: string[] = [];
  let current = '';
...
function emitSvg(scene: SceneData, opts: CliArgs): string {
  ...
  for (const o of scene.objects) {
    if (o.type !== 'highlighter' || o.points.length < 1) continue;
  ...
  for (const o of scene.objects) {
    ...
    parts.push(
      '<line x1="' + o.from.x + '" y1="' + o.from.y + '" x2="' + shaftEndX + '" y2="' + shaftEndY +
    ...
    parts.push(
      '<polygon points="' + ax + ',' + ay + ' ' + bx + ',' + by + ' ' + cx + ',' + cy + '" fill="' + o.color + '"/>',
    );
  }
  // Text labels on top.
  for (const o of scene.objects) {
    ...
    parts.push(
      '<text x="' + o.pos.x + '" y="' + o.pos.y + '" font-size="' + o.fontSize + '" fill="' + o.color +
  ...
  for (let i = 0; i < lines.length; i++) {
    parts.push(
      '<text x="' + bounds.cx + '" y="' + (startY + i * lineHeight) +
...
function emitPng(scene: SceneData, opts: CliArgs): NodeBuffer {
  ...
  for (const o of scene.objects) if (o.type === 'highlighter') drawHighlighter(ctx, o);
  for (const o of scene.objects) if (o.type === 'arrow') drawArrow(ctx, o);
  for (const o of scene.objects) if (o.type === 'text') drawText(ctx, o);
...
function main(): void {
  ...
  if (ext === '.svg') {
    ...
    writeFileSync(resolve(opts.output), Buffer.from(svg, 'utf-8') as unknown as NodeBuffer);
