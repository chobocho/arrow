...
`src/*.ts`를 esbuild로 번들링한 뒤 `release/index.html` 한 파일로 인라인합니다.
최초 1회 `npm install`이 필요합니다(개발 의존성만 — 런타임은 외부 참조 0).
...
- npm 스크립트로도 동일:
  ```sh
  npm run build        # bundle + 인라인
  npm run build:bundle # bundle만
  npm run build:min    # 미니파이 번들 + 인라인
  npm test             # bundle + 노드 테스트
...
├─ dist/bundle.js            # esbuild가 src/index.ts에서 생성 (git ignore)
├─ scripts/build.mjs         # esbuild + 인라인 파이프라인
├─ package.json              # esbuild / typescript dev deps
├─ src/index.ts              # 단일 번들 엔트리 (ArrowApp 글로벌 노출)
├─ src/                      # TypeScript 소스
│  ├─ main.ts
