...
📃 **내보내기** 버튼은 현재 씬을 같은 `.arrow` 형식으로 저장합니다 — 외부 편집기로 다듬거나 텍스트 diff로 변경 사항을 추적할 때 유용합니다.
...
- 📥📃 **`.arrow` 텍스트 포맷 가져오기 / 내보내기**: 한 줄 = 한 체인 (`docs/examples/spec.arrow` 참고). 현재 씬을 같은 형식으로 다시 저장 가능 — 외부 편집 / 버전 관리에 유용
- 🌐 **한국어 / 영어 인터페이스 토글**
- 📱 **PC + 모바일 터치 UI 지원**
