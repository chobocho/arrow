...
// Serialize the current scene back to .arrow text. Reconstructs chains by
// matching each arrow's endpoints to the nearest text node (with a virtual
// "topic" node at canvas center for the centerText). Walks the resulting
// directed graph from roots — the topic when it has outgoing edges plus any
// non-topic node with no incoming edges — emitting one chain per leaf so
// every edge appears at least once. Round-trip with parseArrowFile is
// structure-preserving (positions may shift, edges and labels won't).
// Match radius: how far a text center can be from an arrow endpoint and
// still be considered the source/target. Generous enough to handle the
// import-time trim plus user-resized text, tight enough to skip unrelated
// far-away texts.
const SERIALIZE_MATCH_RADIUS = 220;

export function serializeArrowFile(scene: SceneData): string {
  interface Node {
    label: string;
    center: Vec;
    isTopic: boolean;
  }
  const topicLabel = (scene.centerText && scene.centerText.trim()) || 'Topic';
  const topic: Node = {
    label: topicLabel,
    center: { x: MAX_CANVAS_SIZE / 2, y: MAX_CANVAS_SIZE / 2 },
    isTopic: true,
  };
  const nodes: Node[] = [topic];
  for (const obj of scene.objects) {
    if (obj.type !== 'text') continue;
    const charW = obj.fontSize * 0.65;
    const w = Math.max(charW, charW * obj.text.length);
    const h = obj.fontSize;
    nodes.push({
      label: obj.text,
      center: { x: obj.pos.x + w / 2, y: obj.pos.y + h / 2 },
      isTopic: false,
    });
  }

  const closest = (point: Vec): Node | null => {
    let best: Node | null = null;
    let bestD = SERIALIZE_MATCH_RADIUS;
    for (const n of nodes) {
      const dx = n.center.x - point.x;
      const dy = n.center.y - point.y;
  ...
  for (const obj of scene.objects) {
    ...
    const src = closest(obj.from);
    const dst = closest(obj.to);
    if (!src || !dst || src === dst) continue;
    const outs = outgoing.get(src) as Node[];
    // Dedupe parallel edges between the same pair of nodes — they would
    // produce identical chain text otherwise.
    if (outs.indexOf(dst) >= 0) continue;
    outs.push(dst);
    (incoming.get(dst) as Node[]).push(src);
  }

  const roots: Node[] = [];
  // Topic first when it has outgoing edges so its branches lead the file.
  if ((outgoing.get(topic) as Node[]).length > 0) roots.push(topic);
  for (const n of nodes) {
    if (n === topic) continue;
    if ((incoming.get(n) as Node[]).length === 0 && (outgoing.get(n) as Node[]).length > 0) {
      roots.push(n);
    }
  }

  const chains: string[][] = [];
  const dfs = (node: Node, path: string[], visited: Set<Node>): void => {
    if (visited.has(node)) {
      // Cycle — emit what we have so the edge isn't lost, then bail to
      // avoid infinite recursion.
      chains.push(path.slice());
      return;
    }
    visited.add(node);
    const kids = outgoing.get(node) as Node[];
    if (kids.length === 0) {
      chains.push(path.slice());
    } else {
  ...
  for (const root of roots) {
    dfs(root, [root.label], new Set());
  }

  // Floating text nodes (no incoming, no outgoing) are preserved as
  // single-label chains so round-trip doesn't silently drop them.
  for (const n of nodes) {
    if (n === topic) continue;
    if ((incoming.get(n) as Node[]).length === 0 && (outgoing.get(n) as Node[]).length === 0) {
      chains.push([n.label]);
  ...
  const lines: string[] = ['arrow', topicLabel];
