...
- 동기: 최근 누적된 비명시적 UX(저장 이름 자동 채움, saveAs vs save 의미 차이, 색상·굵기·글자 크기 입력의 selection-aware 동작, 좌측 상단 제목이 주제를 따라감, 글자 모드 클릭 후 자동 선택 전환)가 도움말에 노출되지 않아 사용자가 발견하기 어려움. 한 섹션으로 묶어 명시.
- 변경: i18n 신규 키 `helpSecToolbar` / `helpToolbar` 한·영. `Modals.ts` sections 배열에 모바일과 도구 사이로 삽입.
- 섹션 내용(한국어):
  - 💾 저장 — 이름 미지정 작업은 입력창에 가운데 주제가 미리 채워져 Enter 한 번이면 저장.
  - 📝 새 이름으로 저장 — 사본 저장(새 id). 단순 이름 변경은 작업 목록에서.
  - 색상 / 굵기 / 글자 크기 입력 — 선택된 객체가 있으면 그 객체에 즉시 적용, 없으면 기본값만 변경.
  - 좌측 상단 제목 — 주제 우선, 없으면 작업 이름.
  - 글자 모드 클릭 후 commit → 자동으로 선택 모드 전환.
- 영어판도 동일 내용으로 대응.
