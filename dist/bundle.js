...
(function () {
  ...
  function parseArrowFile(content, sceneName) {
    ...
    var edges = [];
    var seenEdges = {};
    // Attach for layout only: include child in parent.children so the
    // layout pass places it but do NOT emit a parent→child arrow. Used
    // for new-root nodes (free-standing chains).
    function attachForLayoutOnly(parent, child) {
    ...
    for (var k = 0; k < chains.length; k++) {
      ...
      if (!parent) {
        ...
        nodes[first] = parent;
        // Place in topic's fan for layout, but no arrow from topic — the
        // spec treats new start-words as independent roots.
        attachForLayoutOnly(topicNode, parent);
      }
  ...
  // by the promise when clicked. Esc / backdrop resolve null. Enter activates
  // the primary button if one is provided.
  function customChoice(message, buttons) {
    ...
    return new Promise(function (resolve) {
      ...
      document.addEventListener('keydown', onKey, true);
      overlay.addEventListener('mousedown', function (ev) { if (ev.target === overlay) finish(null); });
      requestAnimationFrame(function () { (primaryBtn || actions.firstChild).focus(); });
    });
  }
  ...
  function App(root) {
    ...
    this.input = new InputHandler(this.canvas, this.view, this.store, {
      ...
      onSelect: function (id) { self.selectedId = id; self._updateSelectionUi(); self._syncFontInputToSelection(); self._syncColorInputToSelection(); self._syncThicknessInputToSelection(); },
    ...
    this.store.subscribe(function () {
      ...
      self._syncThicknessInputToSelection();
      self._updateTitle();
      self._requestRender();
    });
  ...
  // so the toolbar reflects the resize target. Falls back to the default used
  // for new arrows/highlighters when text or nothing is selected.
  App.prototype._syncThicknessInputToSelection = function () {
    var el = document.getElementById('inputThickness');
    if (!el) return;
    if (document.activeElement === el) return;
    var sel = this._getSelectedObject();
    if (sel && (sel.type === 'arrow' || sel.type === 'highlighter')) {
      ...
      el.value = String(this.thickness);
    }
  };
  ...
  App.prototype._bindUi = function () {
    ...
    var thickHistoryPushed = false;
    thickEl.addEventListener('focus', function () { thickHistoryPushed = false; });
    thickEl.addEventListener('input', function () {
      var raw = parseFloat(thickEl.value);
  ...
  App.prototype._updateTitle = function () {
    ...
    if (!el) return;
    var scene = this.store.get();
    var topic = String(scene.centerText || '').replace(/^\s+|\s+$/g, '');
  ...
  App.prototype._ensureName = function () {
    ...
    // Placeholder names that should trigger a name prompt — both current
    // language i18n values and legacy literals so older saved scenes
    // and the SceneStore default both upgrade through this prompt.
    var placeholders = [
    ...
    if (!name || placeholders.indexOf(name) >= 0) {
      var self = this;
      // Pre-fill the prompt with the center topic so the most natural
      // name for the work is one Enter-press away.
      var topic = String(this.store.get().centerText || '').replace(/^\s+|\s+$/g, '');
      return customPrompt(t('promptName'), topic).then(function (input) {
  ...
  App.prototype._save = function () {
    var self = this;
    return this._ensureName().then(function (name) {
      if (name === null) return;
  ...
  App.prototype._saveAs = function () {
    var self = this;
    // saveAs makes a COPY (new id, new timestamps) — distinct title so the
    // intent is unambiguous vs. ensureName's first-time naming prompt.
    customPrompt(t('promptSaveAs'), this.store.get().name).then(function (input) {
  ...
  App.prototype._exportPng = function () {
    ...
    a.href = url;
    // Date-stamped filename so multiple exports of the same work in one
    // folder don't overwrite each other.
    var base = this.store.get().name || 'arrow';
    a.download = base + '_' + todayStamp() + '.png';
    a.click();
  };
  // Local-date YYYYMMDD stamp for export filenames. Local (not UTC) so the
  // filename matches the date on the user's clock.
  function todayStamp() {
    var d = new Date();
  ...
  App.prototype._openHelpModal = function () {
    ...
    for (var i = 0; i < sections.length; i++) {
      ...
      // keys. Help strings are hardcoded — no user input — so this is safe.
      // `.ap-help-body { white-space: pre-line }` still turns `\n` into
      // visual line breaks.
      txt.innerHTML = t(sections[i][1]);
